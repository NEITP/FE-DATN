...
import { Link, NavLink, useHistory, useParams } from 'react-router-dom';  // Thư viện để điều hướng trang
import { useSelector, useDispatch } from 'react-redux';  // Dùng để kết nối với Redux Store
import {
...
} from '../../services/userService';  // Các service để gọi API
import './OrderHomePage.scss';  // File CSS
import AddressUsersModal from '../ShopCart/AdressUserModal';  // Modal để thêm địa chỉ người dùng
import { ChooseTypeShipStart, getItemCartStart } from '../../action/ShopCartAction';  // Các action của Redux
import { toast } from 'react-toastify';  // Dùng để thông báo cho người dùng
import storeVoucherLogo from '../../../src/resources/img/storeVoucher.png';  // Hình ảnh logo voucher
import ShopCartItem from '../../component/ShopCart/ShopCartItem';  // Component giỏ hàng
import VoucherModal from '../ShopCart/VoucherModal';  // Modal để chọn voucher
import CommonUtils from '../../utils/CommonUtils';  // Các hàm tiện ích chung
import { EXCHANGE_RATES } from '../../utils/constant';  // Hằng số tỷ giá quy đổi

function OrderHomePage(props) {
    const dispatch = useDispatch();  // Khởi tạo dispatch để dispatch action trong Redux
    const [dataAddressUser, setdataAddressUser] = useState([]);  // Dữ liệu địa chỉ người dùng
    const { userId } = useParams();  // Lấy userId từ URL
    let history = useHistory();  // Lịch sử điều hướng trang
    const [addressUserId, setaddressUserId] = useState('');  // ID của địa chỉ người dùng được chọn
    const [ratesData, setratesData] = useState([]);  // Dữ liệu tỷ giá
    const [priceShip, setpriceShip] = useState(0);  // Giá vận chuyển
    let price = 0;  // Tổng tiền chưa tính giảm giá
    let total = 0;  // Tổng tiền sau khi tính các yếu tố (voucher, phí vận chuyển)
    const [stt, setstt] = useState(0);  // Trạng thái của địa chỉ
    let dataCart = useSelector(state => state.shopcart.listCartItem);  // Dữ liệu giỏ hàng từ Redux Store
    let dataVoucher = useSelector(state => state.shopcart.dataVoucher);  // Dữ liệu voucher từ Redux Store
    let dataTypeShip = useSelector(state => state.shopcart.dataTypeShip);  // Dữ liệu loại vận chuyển từ Redux Store
    const [isChangeAdress, setisChangeAdress] = useState(false);  // Cờ để kiểm tra xem có thay đổi địa chỉ không
    const [isOpenModalAddressUser, setisOpenModalAddressUser] = useState(false);  // Cờ để mở/đóng modal địa chỉ người dùng
    const [isOpenModal, setisOpenModal] = useState(false);  // Cờ để mở/đóng modal voucher
    const [typeShip, settypeShip] = useState([]);  // Dữ liệu loại vận chuyển
    const [activeTypePayment, setactiveTypePayment] = useState(1);  // Loại hình thanh toán (online hay offline)
    const [activeTypeOnlPayment, setactiveTypeOnlPayment] = useState(1);  // Loại hình thanh toán online (VNPAY hay phương thức khác)
    const [note, setnote] = useState('');  // Ghi chú cho đơn hàng

    // useEffect để thực hiện các tác vụ khi component được render
    useEffect(() => {
        dispatch(getItemCartStart(userId));  // Lấy dữ liệu giỏ hàng từ Redux
        let fetchDataAddress = async () => {
            await loadDataAddress(userId);  // Lấy dữ liệu địa chỉ người dùng
        };
        fetchDataAddress();

        // Lấy các loại vận chuyển
        ...
        fetchExchangeRate();  // Lấy tỷ giá
        if (dataTypeShip && dataTypeShip.price) {
            setpriceShip(dataTypeShip.price);  // Set giá vận chuyển
        }

    }, []);

    // Hàm lấy dữ liệu địa chỉ của người dùng
    let loadDataAddress = async (userId) => {
        let res = await getAllAddressUserByUserIdService(userId);
        if (res && res.errCode === 0) {
            setdataAddressUser(res.data);  // Lưu dữ liệu địa chỉ vào state
            setaddressUserId(res.data[0].id);  // Set địa chỉ mặc định
        }
    };

    // Hàm đóng modal địa chỉ người dùng
    ...
    // Hàm nhận dữ liệu từ modal khi thêm địa chỉ
    let sendDataFromModalAddress = async (data) => {
        ...
        if (res && res.errCode === 0) {
            toast.success("Thêm địa chỉ thành công !");  // Thông báo thành công
            await loadDataAddress(userId);  // Cập nhật lại dữ liệu địa chỉ
        } else {
    ...
    let handleOnChange = (id, index) => {
        setaddressUserId(id);  // Cập nhật ID địa chỉ
        setstt(index);  // Cập nhật trạng thái địa chỉ
    };
    ...
    // Hàm đóng modal voucher khi chọn voucher xong
    let closeModalFromVoucherItem = () => {
        setisOpenModal(false);
    };

    // Tính giá trị sau khi áp dụng voucher
    ...
    // Hàm chọn loại vận chuyển
    let handleChooseTypeShip = (item) => {
        dispatch(ChooseTypeShipStart(item));  // Dispatch action chọn loại vận chuyển
        setpriceShip(item.price);  // Cập nhật giá vận chuyển
    };

    // Hàm lấy tỷ giá từ API
    let fetchExchangeRate = async () => {
        let res = await getExchangeRate();
        if (res) setratesData(res);  // Cập nhật tỷ giá
    };

    // Hàm lưu đơn hàng
    let handleSaveOrder = async () => {
        if (!dataTypeShip.id) {
            toast.error("Chưa chọn đơn vị vận chuyển");  // Thông báo lỗi nếu chưa chọn vận chuyển
        } else {
            let result = [];
            dataCart.map((item) => {
                let object = {};
            ...
            // Kiểm tra nếu thanh toán online
            if (activeTypePayment === 0) {
                ...
                if (res && res.errCode === 0) {
                    ...
                    dispatch(getItemCartStart(userId));  // Cập nhật lại giỏ hàng
                    setTimeout(() => {
                        window.location.href = '/user/order/' + userId;  // Điều hướng đến trang đơn hàng
                    }, 2000);
                } else {
                    toast.error(res.errMessage);  // Thông báo lỗi nếu có
                }
            } else {
                // Tính toán tổng tiền khi thanh toán qua cổng online
                total = dataVoucher && dataVoucher.voucherData ? totalPriceDiscount(price, dataVoucher) + priceShip : price + (+priceShip);
                total = parseFloat((total / EXCHANGE_RATES.USD).toFixed(2));  // Quy đổi tiền tệ
                if (activeTypeOnlPayment === 1) {
                    ...
                    if (res && res.errCode === 0) {
                        ...
                        window.location.href = res.link;  // Điều hướng tới trang thanh toán
                    }
                } else {
                    // Nếu thanh toán offline
                    history.push({
